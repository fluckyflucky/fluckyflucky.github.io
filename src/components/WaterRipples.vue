...
onMounted(() => {
  ...
  const ctxResult = canvas.value.getContext('2d')
  if (!ctxResult) return
  const ctx = ctxResult
  ...
  function addTrailAndRipple(x: number, y: number) {
    ...
    if (now - lastTrailSpawn > 16) {
      if (trail.length < MAX_TRAIL) {
        trail.push({ x, y, opacity: 0.4, size: 2 + Math.random() * 2 })
      }
      lastTrailSpawn = now
    }

    if (now - lastRippleSpawn > 200 && ripples.length < MAX_RIPPLES) {
      lastRippleSpawn = now
      ripples.push({
  ...
  function onMouseMove(e: MouseEvent) { addTrailAndRipple(e.clientX, e.clientY) }
  function onMouseClick(e: MouseEvent) { addSplash(e.clientX, e.clientY) }
...
<template>
  <canvas ref="canvas" class="water-ripples" aria-hidden="true" />
</template>
